...
| #CSharp | 🟢 Full | ⚪ Unknown | ⚪ Unknown | ⚪ Unknown | Keysharp-only. Embeds C# compiled into the script's own assembly for hot paths. A block sits at module scope or in a class body. Each script module has its own inline C# syntax tree and module-wide using pool, so aliases and directives do not leak between modules. Public module methods are locally visible and explicitly importable; [Export] enables wildcard import and Default = true sets the default export. Boundary values use AutoHotkey conversions, including wrapped CLR object/value round-tripping; only public property accessors are exposed and init accessors are read-only. Verified on Windows; Linux and macOS are unverified. |
