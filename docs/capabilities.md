...
| Audio.Hooks | 🟢 Full | ⚪ Unknown | ⚪ Unknown | 🟠 Planned | An Array of every live Audio.OnDeviceChange hook, oldest first, from every thread. A snapshot. |
...
| Audio.OnDeviceChange() | 🟢 Full | ⚪ Unknown | ⚪ Unknown | 🟠 Planned | Fires callback(hook, kind, device) when a device is added, removed, renamed or becomes the default, where kind is "Added", "Removed", "Changed" or "DefaultChanged". Returns an Audio.DeviceHook, an EventHook managed exactly like WinEvent and Monitor.OnChange hooks; dropping the handle does not unsubscribe it. |
...
| Clipboard.Hooks | 🟢 Full | 🟢 Full | 🟢 Full | ⚪ Unknown | An Array of every live Clipboard.OnChange hook, oldest first, from every thread. A snapshot; OnClipboardChange handlers are not included. |
...
| Clipboard.OnChange() | 🟢 Full | 🟢 Full | 🟢 Full | ⚪ Unknown | Calls Callback(Hook, Type) with 0 (empty), 1 (text or files), or 2 (other); A_EventInfo has the same value. Returns a ClipboardHook (an EventHook) and runs independently of OnClipboardChange. Native callbacks are unverified on Linux and macOS. |
...
| Clr.Hooks | 🟢 Full | 🟢 Full | 🟢 Full | 🟢 Full | An Array of every live CLR event subscription this script made, oldest first, mixing instance and static events. A snapshot. |
...
| Clr.ManagedInstance.OnEvent() | 🟢 Full | 🟢 Full | 🟢 Full | 🟢 Full | Subscribes to a .NET event on an instance (or a static event on a ManagedType), with the callback run on the subscribing thread. Returns a Clr.EventSubscription, an EventHook; Pause() detaches the delegate and Start() re-attaches it. AddRemove is 1 (add) or 0 (remove); any other value is a ValueError. add_X/remove_X route through the same path. |
...
| EventHook | 🟢 Full | 🟢 Full | 🟢 Full | 🟢 Full | Keysharp-specific Ks base class of every subscription handle (WinEvent hooks, MonitorHook, ClipboardHook, Audio.DeviceHook, Clr.EventSubscription) and of InputHook. Handles come only from the factories, and a subscription lives until Stop(), its thread ending or the script exiting; dropping the handle does not stop it. |
| EventHook.EndReason | 🟢 Full | 🟢 Full | 🟢 Full | 🟢 Full | "" while the hook is active or paused. Once it ends: "Stopped" (Stop() or a third-party detach), "Exit" (its thread or the script ended) or "Failed" (the native event source could not be installed). The first reason is kept. |
| EventHook.InProgress | 🟢 Full | 🟢 Full | 🟢 Full | 🟢 Full | 1 while the hook delivers callbacks; 0 while it is paused or has ended. |
| EventHook.Pause() / .Start() | 🟢 Full | 🟢 Full | 🟢 Full | 🟢 Full | Pause() suspends delivery without unregistering: a callback that comes due while the hook is paused is discarded. Start() resumes it, and does nothing to an active or ended hook. |
| EventHook.Stop() | 🟢 Full | 🟢 Full | 🟢 Full | 🟢 Full | Ends the subscription for good with EndReason "Stopped". Callbacks already queued are discarded; one already running finishes. A one-shot hook calls it as the first statement of its callback. |
...
| InputHook() | 🟢 Full | 🟡 Partial | 🟡 Partial | 🟡 Partial | The InputHook function creates an object which can be used to collect or intercept keyboard input. In Keysharp it is an EventHook, so `ih is EventHook` holds and it gains Pause(). |
...
| InputHook.EndReason | 🟢 Full | 🟡 Partial | 🟡 Partial | 🟡 Partial | Why the input ended: Stopped, Timeout, Match, EndKey, Max or Failed (the hook could not be installed). "" while the input has not started, is in progress or is paused. Unlike AutoHotkey, a hook that was never started reports "", not "Stopped". |
| InputHook.FindAnywhere | 🟢 Full | 🟡 Partial | 🟡 Partial | 🟡 Partial | Matches phrases anywhere in the input buffer. |
| InputHook.InProgress | 🟢 Full | 🟡 Partial | 🟡 Partial | 🟡 Partial | 1 while the input is collecting; 0 before Start(), while paused and after it ends. |
...
| InputHook.Pause() | 🟢 Full | 🟡 Partial | 🟡 Partial | 🟡 Partial | Keysharp extension from EventHook. Suspends an input in progress without ending it: keys pass through uncollected and unsuppressed, no callback runs, and the Timeout clock stops. Start() resumes it; Stop() ends it. |
| InputHook.Start() | 🟢 Full | 🟡 Partial | 🟡 Partial | 🟡 Partial | Starts capturing input, or resumes a paused input with its buffer and remaining Timeout intact; does nothing while one is in progress. Linux and macOS use a suppression-capable global hook, which requires InputMonitoring and InputControl. |
| InputHook.Stop() | 🟢 Full | 🟡 Partial | 🟡 Partial | 🟡 Partial | Ends an input that is in progress or paused with EndReason "Stopped" and calls OnEnd; notifications queued for OnChar, OnKeyDown, OnKeyUp and the mouse callbacks are discarded, while OnEnd always runs. On a hook that was never started it only sets EndReason to "Stopped". |
...
| Monitor.Hooks | 🟢 Full | 🟢 Full | 🟢 Full | 🟢 Full | An Array of every live Monitor.OnChange hook, oldest first, from every thread. A snapshot. |
...
| WinEvent.EventName | 🟢 Full | 🟢 Full | 🟢 Full | 🟢 Full | The event this subscription listens for, e.g. "Active" or "Move". |
...
| WinEvent.Hooks | 🟢 Full | 🟢 Full | 🟢 Full | 🟢 Full | An Array of every live WinEvent hook, oldest first, from every thread. A snapshot: iterate it to pause or stop hooks in bulk. |
