...
| #CSharp | 🟢 Full | 🟢 Full | 🟢 Full | ⚪ Unknown | Keysharp-only. Embeds C# compiled into the script's own assembly for hot paths. A block sits at module scope or in a class body. Public module methods are locally visible and explicitly importable; [Export] enables wildcard import and Default = true sets the default export. Boundary values use AutoHotkey conversions. Platform-neutral implementation verified on Windows and Linux; macOS untested. |
