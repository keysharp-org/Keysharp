...
| Monitor | 🟢 Full | 🟢 Full | 🟢 Full | ⚪ Unknown | Keysharp-specific Ks class for one display: identity and metadata beyond the AHK MonitorGet* functions, plus brightness and DDC/CI control. A Monitor is a snapshot of the topology (call Refresh() to re-read) plus a live handle to the device. macOS is unverified: it is implemented against CoreGraphics and NSScreen but has not been run on a Mac. |
...
| Monitor.Brightness | 🟢 Full | 🟡 Partial | 🟡 Partial | ⚪ Unknown | Get or set the monitor's brightness as a percentage (0-100). Each access is a real device transaction (tens of milliseconds over DDC/CI) and is deliberately not cached, because the monitor's own buttons can change the value too. Windows uses WMI for the built-in panel and DDC/CI for external monitors. Linux uses the kernel backlight class for the built-in panel (a direct sysfs write where permitted, else logind's SetBrightness) and DDC/CI over /dev/i2c-* for external monitors, which needs i2c-dev loaded and access to the bus - hence partial. macOS uses the private DisplayServices framework for the built-in display only and is unverified. Throws an OSError naming the reason where unsupported; test HasBrightness to branch without an exception. |
...
| Monitor.FromWindow() | 🟢 Full | 🟢 Full | 🟡 Partial | ⚪ Unknown | The monitor a window overlaps most. Inherits the platform's foreign-window geometry support, so on Wayland it needs a compositor backend (KWin/GNOME/Cinnamon) to locate the window at all. |
| Monitor.GetVCP() / .SetVCP() | 🟢 Full | 🟡 Partial | 🟡 Partial | 🔴 Unsupported | Read or write a raw DDC/CI VCP feature (MCCS): 0x10 brightness, 0x12 contrast, 0x60 input source, 0x62 speaker volume, 0xD6 power mode. GetVCP returns {current, max}. SetVCP drives the monitor's firmware directly - an input-source or power code will switch the monitor away from this computer, and some displays mishandle codes they document, so verify a code against the monitor's MCCS documentation first. Windows uses dxva2; Linux talks to /dev/i2c-* directly, which needs i2c-dev loaded and bus access. macOS has no supported interface: DDC/CI there requires the private IOAVService, which differs between Apple Silicon and Intel. |
| Monitor.HasBrightness | 🟢 Full | 🟡 Partial | 🟡 Partial | ⚪ Unknown | Whether Brightness works for this monitor. This is a real probe of the device rather than a platform guess, so it costs one brightness read. |
...
| Monitor.Refresh() | 🟢 Full | 🟢 Full | 🟢 Full | ⚪ Unknown | Re-reads this monitor's topology and metadata in place and returns the same object. Matches by name first so the monitor is still tracked after the display order changes, falling back to the index. |
...
| Monitor.X / .Y / .W / .H / .Bounds / .WorkArea | 🟢 Full | 🟢 Full | 🟢 Full | ⚪ Unknown | Geometry in native screen coordinates, the same space MonitorGet reports. Bounds and WorkArea are objects with x, y, w and h. |
