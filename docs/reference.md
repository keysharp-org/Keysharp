...
* The process for reading and running a script is:
	...
	+ Optionally output the generated C# code to a .cs file for debugging purposes with the `--transpile` option, without running the script. The output leaves out the location stamps the compiler adds before each statement for `Error.Stack`; Keyview's Full code view shows the code exactly as compiled.
...
* The `File` object is internally named `KeysharpFile` so that it doesn't conflict with `System.IO.File`. As with `Func` and `Object`, only the AutoHotkey name is usable from a script; the internal name appears only in diagnostics of Keysharp's own C# code.
...
* An `Error` records where it came from when it is constructed, as AutoHotkey v2.1 records it: `What`, `File`, `Line` and `Stack` follow the same rules, and `Stack` has the same form. Keysharp keeps the call stack this needs itself, so it does not depend on debug information or on what the .NET JIT compiler inlines.
	+ A script run from its source quotes the code of each line in `Stack` and in the error dialog, as AutoHotkey does. Compiled output, from `--compile exe` or `--compile asm`, does not carry the source, so its `Stack` gives each frame's file, line and function without the code.
	+ Compiled output never records a full path. It names each file in error diagnostics by where it resolved: `./Name.ahk` for a file in the script's folder or below it, such as `./Lib/Name.ahk`, `<UserLib>/Name.ahk` and `<StdLib>/Name.ahk` for the user and standard libraries, and `<External>/Name.ahk` for any other file. If two files would have the same name, the later one gains a number before its extension, such as `<External>/Name (2).ahk`. In a `.cks`, `A_LineFile` on an included line resolves the included file's path relative to the `.cks` output directory at compile time, then relative to the `.cks` directory at runtime. Moving the `.cks` without the same relative source layout changes that value. If the included file is on another drive and cannot have a relative path, `A_LineFile` is `A_ScriptFullPath`. Main-script lines in a `.cks`, and every line in a standalone compiled executable, report `A_ScriptFullPath` instead.
	+ A thread appears in `Stack` under the name AutoHotkey gives it, such as the hotkey's name, `Timer`, `Callback`, `Gui` or `Menu`. Threads AutoHotkey has no equivalent of appear as `Event`, `WinEvent`, `Com`, `Clr` or `RealThread`.
	+ A member of a script's `#CSharp` code appears in `Stack` as a built-in function does, at the line which called it.
...
* The standalone `#ErrorStdOut` directive sends subsequent load-time errors and uncaught runtime errors to standard error instead of a dialog. A runtime error is written as `File (Line) : ==> Message`, followed by a `     Specifically: Extra` line when `Extra` is not empty, the form AutoHotkey writes a load-time error in, which editors can jump to the line from. AutoHotkey itself always shows the dialog for a runtime error. The `--errorstdout` command-line switch applies to load-time errors from the start of loading. A GUI-subsystem process's error stream is visible when captured by piping or redirection. For example:
	...
	* Boolean property naming:
	...
	+ New preprocessor directives:
		...
		+ `#CSharp` embeds C# members in the script assembly for hot loops, buffer work and interop.
			...
			+ Values use the same conversions as `Ks.Clr`. Unsupported public signatures are rejected, and CLR exceptions are mapped to catchable Keysharp errors where possible, whose `What` names the member and whose `Line` is the line which called it.
