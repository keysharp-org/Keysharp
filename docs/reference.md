...
* Miscellaneous behavior:
	...
	* **Boolean property naming.** On a class, an `Is` prefix marks a **read-only predicate** (`Thread.IsActive`, `Thread.IsInterruptible`, `Func.IsBuiltIn`, `Func.IsVariadic`); a **settable** boolean is named for the state itself (`Thread.Critical`, `Thread.Paused`, `WinEvent.Paused`), so it reads as `obj.Paused := true` rather than as a question.
	...
	+ New preprocessor directives:
		+ `#CSharp` embeds C# directly in a script, compiled into the script's own assembly. It exists for the handful of paths where AHK-level semantics cost too much — tight numeric loops, buffer and pixel work, repeated P/Invoke.
			+ Two forms, both allowed only at the top level of a script or module:
				```
				#CSharp
				using System.Text;
				...
				```
				and `#CSharp "helper.cs"`, which reads the file instead. A relative path resolves against the directory of the file the directive was written in, like a relative `#include`. Reach for the file form once the C# outgrows a block: a real `.cs` gets syntax highlighting and IntelliSense that inline text never will.
			+ **Visibility is plain C# accessibility.** A `public static` method becomes a callable script function under its own name; anything not `public` is a private helper the script cannot see. There is no export attribute to learn.
			+ **Arguments convert with AutoHotkey's rules, not the CLR's.** A typed parameter accepts ordinary script values — `SumSquares("4")` and `SumSquares(4.0)` both work, matching `"4" == 4` — and only a genuinely non-numeric value raises, as a catchable `TypeError`. `ref`, `out`, `in` and pointer parameters cannot be passed from a script and are rejected at compile time; make such a method non-public if it was never meant to be called from one.
			+ **Errors point at your code.** A C# compile error is reported against the script's own file and line, and an exception thrown inside inline C# arrives as the matching Keysharp error, so an ordinary `try`/`catch IndexError` intercepts it.
			+ Module globals are readable and writable from C# as static fields, in lowercase (`Hits` is `hits`). Passing a global as a parameter is both faster and safer than reading the field, because the parameter is converted once at the boundary.
			+ A `using` at the top of a block is hoisted; `System`, `System.Collections.Generic`, `System.Runtime.CompilerServices`, `System.Runtime.InteropServices` and `Keysharp.Runtime` are already in scope. `Keysharp.Builtins` deliberately is not, because its `Array`, `String` and `Buffer` would be ambiguous with the `System` types.
			+ The script's preprocessor symbols apply inside the block, so `#if WINDOWS` means the same thing on both sides. (Keysharp matches `#define` names case-insensitively; C# does not.)
			+ `unsafe` code is allowed, with C#'s own `unsafe` keyword as the only opt-in, exactly as in any other C# project — the directive itself takes no options. Mind what that buys you: an out-of-bounds pointer write raises an `AccessViolationException`, which .NET does not deliver to a managed `catch`, so the process dies with no `OnError`, no dialog and no `OnExit`. That is the same exposure `DllCall` and `NumPut` already carry.
			+ Calls into inline C# are dispatched like any other script function, so the gain is in the *body*, not the call. Put the whole loop inside the C#; one call that runs a million iterations is the win, a million calls into a one-line helper is not.
			+ For bulk numeric data, hand C# a `Buffer` rather than an `Array`: `Array` element access goes through the same conversion machinery a script does, so it stays script-speed even from C#, while a `Buffer` can be walked as a `Span`.
			+ Referencing the wider framework (`Console`, `Regex`, `Task`, …) is enabled only for scripts that use this directive, so scripts that do not keep their existing compile speed.
