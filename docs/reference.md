...
* Function objects behave mostly the same as in AutoHotkey.
	...
	+ A callback parameter accepts a function or any object with a callable `Call` member. Pass a function by reference, such as `SetTimer(MyFunc)`; resolve a name known only at run time first with `SetTimer(%"MyFunc"%)`.
...
* New classes:
	...
	+ `Thread`: The current pseudo-thread, obtained from `A_Thread`.
		+ `Thread` is a **class**, not a function, and calling it runs the AHK sub-functions unchanged — `Thread "NoTimers"`, `Thread "Priority", n`, `Thread "Interrupt", n`. One name therefore covers the thread settings and the thread object, which is what lets `A_Thread`'s type simply be `Thread`. It stays a global name (no import needed for `thr is Thread`) because `Thread` was already global as a function; the consequence is that `Thread is Func` is now false.
			...
			```
		+ Pseudo-thread state is pooled and reused, so a `Thread` object captures its ID and re-checks it on every access. Once its pseudo-thread ends, `Id` and `Index` still answer and `IsActive` reports false, while everything else throws `TargetError` — a stored object can never silently describe a later pseudo-thread that reused the slot.
		...
		+ `Exit(n)`, or `thr.Exit(n)` on a Thread object, leaves n as the process exit code if the script exits when that thread ends; a launched thread's code is otherwise dropped, while the auto-execute section's stays pending until the next thread ends.
