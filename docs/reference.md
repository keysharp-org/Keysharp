...
* An `Error` records where it came from when it is constructed, as AutoHotkey v2.1 records it: `What`, `File`, `Line` and `Stack` follow the same rules, and `Stack` has the same form. Keysharp keeps the call stack this needs itself, so it does not depend on debug information or on what the .NET JIT compiler inlines.
	...
	+ Compiled output never records a full path. It names each file by where it resolved: `./Name.ahk` for a file in the script's folder or below it, such as `./Lib/Name.ahk`, `<UserLib>/Name.ahk` and `<StdLib>/Name.ahk` for the user and standard libraries, and `<External>/Name.ahk` for any other file. If two files would have the same name, the later one gains a number before its extension, such as `<External>/Name (2).ahk`. `A_LineFile` in compiled output is `A_ScriptFullPath`, the executable or `.cks`, as AutoHotkey reports the executable for every line of a compiled script.
