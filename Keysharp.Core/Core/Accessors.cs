...
public static class Accessors
{
		...
		get
		{
			...
			static string act()
			{
				...
				if (WindowsAPI.OpenClipboard(A_ClipboardTimeout.Al()))//Will need a cross platform version of this.//TODO
				{
					...
					if (Clipboard.TryGetData(DataFormats.Text, out string text))
						return text;

					if (Clipboard.TryGetData(DataFormats.Html, out string html))
						return html;

					if (Clipboard.TryGetData(DataFormats.Rtf, out string rtf))
						return rtf;

					if (Clipboard.TryGetData(DataFormats.SymbolicLink, out string sym))
						return sym;

					if (Clipboard.TryGetData(DataFormats.UnicodeText, out string uni))
						return uni;

					if (Clipboard.TryGetData(DataFormats.OemText, out string oem))
						return oem;

					if (Clipboard.TryGetData(DataFormats.CommaSeparatedValue, out string csv))
						return csv;
	...
	public static object A_IconTip
	{
		...
		set
		{
			...
			_ = (script.Tray?.Text = value.ToString());
		}
	}
	...
	public static bool A_IsCompiled
	{
		get
		{
			...
			return path is not "keysharp.dll" and not "keysharp.exe" and not "testhost.exe"
				   and not "keysharp" and not "testhost" and not "testhost.dll";
		}
	}
		...
		get
		{
			...
			foreach (var l in s)//Since loop is a stack, this goes in reverse order, which is what we want.
			{
				switch (l.type)
				{
					...
					case LoopType.Each:
					{
						return l.result is not object[] so ? DefaultObject : so.Length > 0 ? so[1] : DefaultObject;
					}
				}
			}
	...
	public static string A_ScriptDir
	{
		get
		{
			...
			return A_IsCompiled
				...
				: script.scriptName == "*" ? A_WorkingDir as string : Path.GetDirectoryName(script.scriptName);
		}
	}
	...
	internal static string CoordModeTypeToString(CoordModeType mode)
	{
		return mode switch
		{
			CoordModeType.Client => "Client",
			CoordModeType.Window => "Window",
			CoordModeType.Screen => "Screen",
			_ => DefaultErrorString,
		};
	}
}
...
public static partial class KeysharpEnhancements
{
	...
	public static string A_DefaultHotstringSendRaw
	{
		get
		{
			return TheScript.HotstringManager.hsSendRaw switch
			{
				SendRawModes.NotRaw => "NotRaw",
				SendRawModes.Raw => "Raw",
				SendRawModes.RawText => "RawText",
				_ => DefaultErrorString,
			};
		}
	}
...
internal static class ThreadAccessors
{
	...
	internal static object A_FileEncoding
	{
		...
		set => Script.TheScript.Threads.CurrentThread.configData.fileEncoding = value is Encoding enc ? enc : Files.GetEncoding(value.ToString());
	}
