...
Check(Crypt.MD5("abc") == "900150983CD24FB0D6963F7D28E17F72")
Check(Crypt.SHA1("abc") == "A9993E364706816ABA3E25717850C26C9CD0D89D")
Check(Crypt.SHA256("abc") == "BA7816BF8F01CFEA414140DE5DAE2223B00361A396177A9CB410FF61F20015AD")
Check(Crypt.SHA384("abc") == "CB00753F45A35E8BB5A03D699AC65007272C32AB0EDED1631A8B605A43FF5BED8086072BA1E7CC2358BAECA134C825A7")
Check(Crypt.SHA512("abc") == "DDAF35A193617ABACC417349AE20413112E6FA4E89A97EA20A9EEEE64B55D39A2192992A274FC1A836BA3C23A3FEEBBD454D4423643CE80E2A9AC94FA54CA49F")
Check(Crypt.SHA256("") == "E3B0C44298FC1C149AFBF4C8996FB92427AE41E4649B934CA495991B7852B855")

; Hash defaults to SHA256, and takes the algorithm name in any case and with the "-" external tools print.
Check(Crypt.Hash("abc") == Crypt.SHA256("abc"))
Check(Crypt.Hash("abc", "sha-256") == Crypt.SHA256("abc"))

; The encoding a string is taken in is selectable, and this is the digest the old UTF-16 behavior produced.
Check(Crypt.SHA256("abc", "UTF-16") == "13E228567E8249FCE53337F25D7970DE3BD68AB2653424C7B8F9FD05E33CAEDF")

; CRC32 as the integer it is, and as the hexadecimal Hash returns for it.
Check(Crypt.CRC32("abc") == 891568578)
Check(Crypt.Hash("abc", "CRC32") == "352441C2")

; Bytes already in memory are hashed as they stand, with no encoding involved.
buf := Buffer(3)
NumPut("UChar", 0x61, "UChar", 0x62, "UChar", 0x63, buf)
Check(Crypt.SHA256(buf) == Crypt.SHA256("abc"))
...
Check(raw.Size == 108000)
Check(Crypt.HashFile(big) == Crypt.SHA256(raw))
Check(Crypt.HashFile(big, "MD5") == Crypt.MD5(raw))
Check(Crypt.HashFile(big, "CRC32") == Crypt.Hash(raw, "CRC32"))
FileDelete big

; Errors from the hashing surface, each of which must be catchable.
Check(Throws(() => Crypt.Hash("abc", "SHA3"), ValueError))
Check(Throws(() => Crypt.HashFile(A_Temp A_DirSeparator "keysharp-crypt-no-such-file.bin"), OSError))
Check(Throws(() => Crypt.HashFile(""), ValueError))
...
Check(Throws(() => Crypt.SHA256("abc", "no-such-encoding"), ValueError))
...
Check(StrGet(plain, plain.Size, "UTF-8") == "hello")
...
Check(!CompareBuffers(secret, again))
Check(secret.Size == 32)
Check(Crypt.Decrypt(again, "key").Size == 5)
...
Check(Base64Encode(Crypt.Encrypt("hello", "key", , , "0123456789abcdef")) == "g1TwShPvFGwTdenQrf+wew==")
Check(Base64Encode(Crypt.Encrypt("hello", "key", "AES", "ECB")) == "V/rlD0HpNugob35tzHnxCw==")
...
Check(StrGet(fixedPlain, fixedPlain.Size, "UTF-8") == "hello")
Check(fixed.Size == 16)
Check(Throws(() => Crypt.Decrypt(fixed, "key", , , "fedcba9876543210"), ValueError))
Check(Throws(() => Crypt.Encrypt("hello", "key", , , "too short"), ValueError))
Check(Throws(() => Crypt.Encrypt("hello", "key", "AES", "ECB", "0123456789abcdef"), ValueError))
...
Check(StrGet(plainEcb, plainEcb.Size, "UTF-8") == "hello")
Check(!CompareBuffers(ecb, secret))
Check(Throws(() => Crypt.Encrypt("hello", "key", "Twofish"), ValueError))
Check(Throws(() => Crypt.Encrypt("hello", "key", "AES", "XTS"), ValueError))
...
Check(Throws(() => Crypt.Decrypt(fixed, "wrongkey", , , "0123456789abcdef"), ValueError))
Check(Throws(() => Crypt.Decrypt("hello", "key"), ValueError))

; RandomBytes is what makes a vector available to a script at all.
iv1 := Crypt.RandomBytes(16)
iv2 := Crypt.RandomBytes(16)
Check(iv1.Size == 16)
Check(!CompareBuffers(iv1, iv2))
Check(Crypt.RandomBytes(0).Size == 0)
...
Check(sealed.Size == 33)
Check(StrGet(opened, opened.Size, "UTF-8") == "hello")
...
for offset in [14, 20, 3] {
    ...
    Check(Throws(() => Crypt.Decrypt(torn, "key", , "GCM"), ValueError))
}

; A nonce is 12 bytes, not the 16 a chaining mode's vector takes.
Check(Throws(() => Crypt.Encrypt("hello", "key", , "GCM", "0123456789abcdef"), ValueError))

; PBKDF2 against RFC 6070's second test vector, which pins the iteration count and the output length.
Check(Base64Encode(Crypt.PBKDF2("password", "salt", 2, 20, "SHA1")) == "6mwBTcctb4zNHtkqzh1B8NjeiVc=")
...
Check(StrGet(opened2, opened2.Size, "UTF-8") == "secret")
Check(Throws(() => Crypt.PBKDF2("password", "salt", 2, 20, "MD5"), ValueError))
Check(Throws(() => Crypt.PBKDF2("password", "salt", 0), ValueError))

; A name left blank falls back to the default, since an omitted argument reaches a function as "".
blank := ""
Check(Crypt.Hash("abc", blank) == Crypt.SHA256("abc"))
Check(Crypt.Hash("abc", blank, blank) == Crypt.SHA256("abc"))
...
Check(hits)
Check(Crypt.SecureRandom(7, 7) == 7)
